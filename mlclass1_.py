...
## Reshape Train Dataset to ( ,30,30,3) and anaylising the output differences
x_train_new = []
...
for i in range(len(x_train)):
    ...
    if y_train[i] == 1:
        ...
        count_spot_train += 1
x_train_new = np.array(x_train_new) # X_Train array reshaped ( , 30,30,3)

## Reshape Test Dataset to ( ,30,30,3)
x_test_new = []
for i in range(len(x_test)):
    x_test_new.append(x_test[i].reshape(30,30,3))
    
x_test_new = np.array(x_test_new) # X_Test array reshaped ( , 30,30,3)
